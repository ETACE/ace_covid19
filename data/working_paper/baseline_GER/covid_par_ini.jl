...
policies[tpol] = "policy_bailout.jl"
policies[tpolout] = "policy_baseline_end.jl"
policies[vacctime] = "policy_allout.jl"

detfrac = 0.15 # detection frequency
adaptivepolicythreshold = 10 * nhh / (detfrac*100000) # Threshold infected/week
poladjfrac = 0.6 # factor by which gap in policy parameter is closeed during phase in / phase out every week

tadaptivepolicystart = tpol + 1149 # Enable adaptive policy response
...
adaptivepolicybad = "policy_bailout.jl" #  policy activated if number of infected below threshold
...
#ininum = Int(ceil(inifrac*nhh))
#iniinf = [[ininum,ininum]]
