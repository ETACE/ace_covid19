...
path_to_data = "../data/no_mutation_pinf125/"
...
include("../data/no_mutation_pinf125/beta_l10/covid_par_ini.jl") # necessary for some basic parameter values (nhh)
...
scatter!([point_A_x], [point_A_y-0.003], markersize = 0, markercolor = [:white], series_annotations = text.(["B"], :top))

scatter!([point_B_x], [point_B_y], markersize = 4, markercolor = [:green])
scatter!([point_B_x], [point_B_y-0.02], markersize = 0, markercolor = [:white], series_annotations = text.(["C"], :bottom))
...
scatter!([point_E_x+.05], [point_E_y], markersize = 0, markercolor = [:white], series_annotations = text.(["D"], :left))



ylims!(plarrows,0.0,0.25)
xlims!(plarrows,0,6)

savefig(plarrows,"$(filename_prefix)variation_policy_no_mutation_pinf125.pdf")
