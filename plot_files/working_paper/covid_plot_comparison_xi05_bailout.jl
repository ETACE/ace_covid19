...
filename_prefix = "..//..//data//working_paper//bailout//xi05//"
plotname_prefix = "..//..//figures//working_paper//bailout_xi05_comparison_" # store the plots here
...
suppfig2b = quiver(gdploss_means[1:ll-1],totcas_means[1:ll-1],quiver=(gdploss_means[2:ll]-gdploss_means[1:ll-1],totcas_means[2:ll]-totcas_means[1:ll-1]), color = [:red], linewidth = 2, xlabel = "Average GDP loss [%]", ylabel = "Mortality [%]")
scatter!(suppfig2b, gdploss_means, totcas_means, markersize = 5, markercolor = [:red], legend=false)


ylims!(suppfig2b,0.008,0.065)
xlims!(suppfig2b,0,10)
...
quiver!(suppfig2b, gdploss_means4[1:ll-1],totcas_means4[1:ll-1],quiver=(gdploss_means4[2:ll]-gdploss_means4[1:ll-1],totcas_means4[2:ll]-totcas_means4[1:ll-1]), color = [:blue], linewidth = 2)
scatter!(suppfig2b, gdploss_means4, totcas_means4, markersize = 5, markercolor = [:blue], legend=false)



### add specific points A B C
scatter!(suppfig2b, [point_A_x], [point_A_y], markersize = 6, markercolor = [:black])
annotate!(suppfig2b,[point_A_x+0.4], [point_A_y+0.0005], text.([L"A_2"]) , fontsize=60 )

scatter!(suppfig2b, [point_B_x], [point_B_y], markersize = 6, markercolor = [:black])
annotate!(suppfig2b,[point_B_x+0.4], [point_B_y+0.0015], text.([L"B_2"]) , fontsize=160 )

scatter!(suppfig2b, [point_C_x], [point_C_y], markersize = 6, markercolor = [:black])
annotate!(suppfig2b,[point_C_x+0.4], [point_C_y], text.([L"C_2"]) , fontsize=160 )


plot!(suppfig2b, size=(600,400))
savefig(suppfig2b,"$(plotname_prefix)supp_fig_2b.pdf")
